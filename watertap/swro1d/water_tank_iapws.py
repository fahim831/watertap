...
def main():
    ...
    hin = iapws95.htpx(T=298*pyo.units.K, P=1e5*pyo.units.Pa)
    ...
    m.fs.wtank.inlet.flow_mol[:].fix(1.0)
    m.fs.wtank.inlet.pressure[:].fix(1e5)
    m.fs.wtank.inlet.enth_mol[:].fix(hin)
    ...
    # m.fs.wtank.display()
    m.fs.wtank.report()
    print(pyo.value(m.fs.wtank.outlet.enth_mol[0]))
    ...
    print('Outlet flow enthalpy, molar flow rate, pressure: ', pyo.value(m.fs.wtank.outlet[:].enth_mol[0])[0], pyo.value(m.fs.wtank.outlet[:].flow_mol[0])[0], pyo.value(m.fs.wtank.outlet[:].pressure[0])[0])
    ...
    if operating_scenario == OperatingScenario.dynamic:
        ...
        ax[1].set_ylabel("Out flow_mol (m)", fontsize=14)
        ax[2].set_ylabel("Out enth_mol (m)", fontsize=14)
        ax[3].set_ylabel("Out pressure (m)", fontsize=14)
